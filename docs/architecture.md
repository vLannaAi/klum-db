...
- **Independent `pre.N` lines.** noy-db and klum-db version separately, on unrelated `major.minor` lines. klum pins noy via a peer-dep **range**, so noy can advance within range without forcing a bump here. **Read the current range and dev pin from `package.json`, never from this file** — a version written into prose has nothing that checks it, and the ones that used to be here were three minor lines stale.
...
- **Optional tooling peers:** `@noy-db/in-devtools`, `@noy-db/to-meter` and `@noy-db/on-magic-link` are `peerDependenciesMeta.optional` — devtools/meter only for consumers using `groupInspector`/`meterGroup`/the `klum` CLI, on-magic-link only for `provisionPortal` invites; none needed by a consumer that just uses `VaultGroup`. *Optional* means a consumer may omit the package, **not** that its declared range may be wrong — `scripts/check-peer-floor.mjs` compiles against all three floors like any other peer.
